...
const SignUp = () => {
    ...
    const handleFormSubmit = async (event) => {
        ...
        const signUpPromise = await fetchHookWithBody('auth/sign-up', { method: 'POST' }, signUpStateObj);
                ...
                <div className="mt-10">
                    <form action="#" onSubmit={handleFormSubmit}>
                        <div className="flex flex-col mb-5">
                            <label htmlFor="firstName" className="mb-1 text-xs tracking-wide text-gray-600">First Name:</label>
                            <div className="relative">
                                <div className=" inline-flex items-center justify-center absolute left-0 top-0 h-full w-10 text-gray-400">
                                    <i className="fas fa-at text-blue-500"></i>
                                </div>

                                <input id="firstName" type="text" required value={signUpStateObj.fristName} onChange={handleInputChange} name="firstName" className=" text-sm placeholder-gray-500 pl-10 pr-4 rounded-2xl border border-gray-400 w-full py-2 focus:outline-none focus:border-blue-400" placeholder="Enter your email" />
                            </div>
                        </div>
                        <div className="flex flex-col mb-5">
                            <label htmlFor="lastName" className="mb-1 text-xs tracking-wide text-gray-600">Last Name:</label>
                            <div className="relative">
                                <div className=" inline-flex items-center justify-center absolute left-0 top-0 h-full w-10 text-gray-400">
                                    <i className="fas fa-at text-blue-500"></i>
                                </div>

                                <input id="lastName" type="text" required value={signUpStateObj.lastName} onChange={handleInputChange} name="lastName" className=" text-sm placeholder-gray-500 pl-10 pr-4 rounded-2xl border border-gray-400 w-full py-2 focus:outline-none focus:border-blue-400" placeholder="Enter your email" />
                            </div>
                        </div>
                        <div className="flex flex-col mb-5">
            ...
            <div className="flex justify-center items-center mt-6">
                <a href="#" target="_blank" className=" inline-flex items-center text-gray-700 font-medium text-xs text-center ">
                    <span className="ml-2">Already have an account?
                        <a href="#" className="text-xs ml-2 text-blue-500 font-semibold" >Login now</a></span>
                </a>
            </div>
        </div>
    )
